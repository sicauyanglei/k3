    ...
    <el-card v-loading="loading">
      <template #header>
        <div class="card-header">
          <span>应付账款账龄分析</span>
          <span class="total-info">应付总额: {{ formatMoney(totalAmount) }}</span>
        </div>
      </template>
...
async function loadData() {
  ...
  try {
    const invoices = await window.api.database.query(
      `SELECT i.supplier_id, s.name as supplier_name, i.total_amount, i.invoice_date,
       (SELECT COALESCE(SUM(p.amount), 0) FROM ap_payment p JOIN ap_write_off w ON p.id = w.payment_id WHERE w.invoice_id = i.id) as paid_amount
       FROM ap_invoice i 
       LEFT JOIN bd_supplier s ON i.supplier_id = s.id
    ...
    for (const inv of invoices) {
      ...
      if (!supplierMap.has(inv.supplier_id)) {
        supplierMap.set(inv.supplier_id, {
      ...
      const supplier = supplierMap.get(inv.supplier_id)
      const invoiceDate = new Date(inv.invoice_date)
      const daysDiff = Math.floor((asOfDate.getTime() - invoiceDate.getTime()) / (1000 * 60 * 60 * 24))
      
      if (daysDiff <= 0) {
        supplier.current += remaining
      } else if (daysDiff <= 30) {
        supplier.days30 += remaining
      } else if (daysDiff <= 60) {
        supplier.days60 += remaining
      } else if (daysDiff <= 90) {
        supplier.days90 += remaining
      } else {
        supplier.over90 += remaining
      }
      supplier.total += remaining
    }
    
    tableData.value = Array.from(supplierMap.values()).filter(s => s.total > 0)
  } catch (error: any) {
...
function handleExport() {
  ...
  if (exportToExcel(exportData, '应付账款账龄分析')) {
