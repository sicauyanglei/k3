...
async function loadData() {
  loading.value = true
  try {
    let sql = `SELECT i.*, s.name as supplier_name FROM ap_invoice i LEFT JOIN bd_supplier s ON i.supplier_id = s.id WHERE 1=1`
    if (searchForm.invoiceNo) {
      sql += ` AND i.invoice_no LIKE '%${searchForm.invoiceNo}%'`
    }
    if (searchForm.supplierId) {
      sql += ` AND i.supplier_id = ${searchForm.supplierId}`
    }
...
function handleReset() {
  Object.assign(searchForm, { invoiceNo: '', supplierId: null, dateRange: [] })
  loadData()
}
...
async function handleDelete(row: any) {
  try {
    ...
    await window.api.database.execute(`DELETE FROM ap_invoice WHERE id = ${row.id}`)
  ...
  try {
    ...
    if (editingItem.value) {
      await window.api.database.execute(
        `UPDATE ap_invoice SET invoice_no='${escapeSql(form.invoiceNo)}', invoice_type='${form.invoiceType}', supplier_id=${form.supplierId}, invoice_date='${form.invoiceDate}', amount=${form.amount}, tax_amount=${form.taxAmount}, total_amount=${form.totalAmount} WHERE id=${editingItem.value.id}`
      )
    } else {
      await window.api.database.execute(
        `INSERT INTO ap_invoice (invoice_no, invoice_type, supplier_id, invoice_date, amount, tax_amount, total_amount, status) VALUES ('${escapeSql(form.invoiceNo)}', '${form.invoiceType}', ${form.supplierId}, '${form.invoiceDate}', ${form.amount}, ${form.taxAmount}, ${form.totalAmount}, 'draft')`
      )
    }
