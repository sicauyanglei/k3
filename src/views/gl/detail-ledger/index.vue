<template>
  <div class="page-container">
    ...
    <el-card v-loading="loading">
      ...
      <el-table :data="tableData" border stripe show-summary :summary-method="getSummary" v-if="tableData.length > 0" size="small">
        <el-table-column prop="voucher_date" label="日期" width="100" />
        <el-table-column prop="voucher_word" label="字" width="50" />
        <el-table-column prop="voucher_no" label="号" width="60" />
...
async function loadAccounts() {
  const data = await window.api.database.query(
    "SELECT id, code, name, direction FROM bd_account WHERE is_enabled = 1 AND is_leaf = 1 ORDER BY code"
  )
...
async function handleSearch() {
  ...
  try {
    ...
    const entries = await window.api.database.query(
      `SELECT e.*, v.voucher_word, v.voucher_no, v.voucher_date, v.year, v.period 
       ...
       AND v.status = 'posted'
       ORDER BY v.voucher_date, v.voucher_no, e.entry_no`
    )
    
    let beginBalance = 0
    const prevPeriod = searchForm.startPeriod - 1
    const prevYear = prevPeriod === 0 ? searchForm.year - 1 : searchForm.year
    const actualPrevPeriod = prevPeriod === 0 ? 12 : prevPeriod
    
    const firstPeriodData = await window.api.database.query(
      `SELECT * FROM gl_balance WHERE account_id = ${searchForm.accountId} AND year = ${prevYear} AND period = ${actualPrevPeriod}`
    )
...
function handlePrint() {
  ...
  const columns = [
    { field: 'voucher_date', title: '日期', width: '12%' },
    { field: 'voucher_word', title: '字', width: '6%' },
    { field: 'voucher_no', title: '号', width: '8%' },
  ...
  const title = `明细账 - ${currentAccount.value?.code || ''} ${currentAccount.value?.name || ''}`
...
function handleExport() {
  ...
  const filename = `明细账_${currentAccount.value?.code || ''}_${searchForm.year}年${searchForm.startPeriod}-${searchForm.endPeriod}期`
